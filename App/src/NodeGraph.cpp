...
namespace App
{
	...
	bool NodeGraph::RemoveNode( NodeId nodeId )
	{
		...
		for( const NodePin& pin : node->inputs ) pins.insert( pin.id );
		for( const NodePin& pin : node->outputs ) pins.insert( pin.id );
		links_.erase( std::remove_if( links_.begin(), links_.end(), [&pins]( const GraphLink& link )
		{
			return pins.contains( link.inputPin ) || pins.contains( link.outputPin );
		} ), links_.end() );
		nodes_.erase( node );
		return true;
	}
	...
	bool NodeGraph::Connect( PinId firstPin, PinId secondPin, std::string* error )
	{
		...
		if( !first || !second )
		{
			if( error ) *error = "El pin no existe.";
			return false;
		}
		...
		{
			if( error ) *error = "Conecta una salida con una entrada.";
			return false;
		}
		if( output->type != input->type )
		{
			if( error ) *error = std::string( "Tipos incompatibles: " ) + NodeValueTypeName( output->type ) + " -> " + NodeValueTypeName( input->type );
			return false;
		}
		if( output->nodeId == input->nodeId || WouldCreateCycle( output->nodeId, input->nodeId ) )
		{
			if( error ) *error = "La conexion crearia un ciclo.";
			return false;
		}
	...
	bool NodeGraph::RemoveLink( LinkId linkId )
	{
		...
		if( link == links_.end() ) return false;
		links_.erase( link );
		return true;
	}
	...
	const NodePin* NodeGraph::FindPin( PinId pinId ) const
	{
		...
		{
			for( const NodePin& pin : node.inputs ) if( pin.id == pinId ) return &pin;
			for( const NodePin& pin : node.outputs ) if( pin.id == pinId ) return &pin;
		}
		return nullptr;
	}
	...
	std::vector<GraphNode>& NodeGraph::Nodes() noexcept { return nodes_; }
	const std::vector<GraphNode>& NodeGraph::Nodes() const noexcept { return nodes_; }
	const std::vector<GraphLink>& NodeGraph::Links() const noexcept { return links_; }
	...
	{
		if( std::find( stack.begin(), stack.end(), nodeId ) != stack.end() ) return Error( "Ciclo detectado durante la evaluacion." );
		...
		if( node->kind == NodeKind::FloatConstant ) return finish( FloatResult( node->constantValue ) );
		if( node->kind == NodeKind::TextureSource ) return finish( TextureResult( node->textureValue ) );
		if( node->kind == NodeKind::TextureCpuModifier )
		{
			const EvaluationResult input = EvaluateInput( node->inputs[0], stack );
			if( !input.valid || !std::holds_alternative<TextureReference>( input.value ) ) return finish( Error( "Modificar textura CPU necesita Texture2D." ) );
			return finish( input );
		}
		if( node->kind == NodeKind::TexturePreview )
		{
			const EvaluationResult input = EvaluateInput( node->inputs[0], stack );
			if( !input.valid ) return finish( input );
			if( !std::holds_alternative<TextureReference>( input.value ) ) return finish( Error( "Visualizar textura necesita Texture2D." ) );
			return finish( input );
		}
		if( node->kind == NodeKind::ComposeVector3 )
		{
			const EvaluationResult x = EvaluateInput( node->inputs[0], stack );
			if( !x.valid ) return finish( x );
			const EvaluationResult y = EvaluateInput( node->inputs[1], stack );
			if( !y.valid ) return finish( y );
			const EvaluationResult z = EvaluateInput( node->inputs[2], stack );
			if( !z.valid ) return finish( z );
		...
		if( node->kind == NodeKind::VectorLength )
		{
			const EvaluationResult input = EvaluateInput( node->inputs[0], stack );
			if( !input.valid ) return finish( input );
			if( !std::holds_alternative<Float3>( input.value ) ) return finish( Error( "VectorLength necesita un Vector3." ) );
		...
		if( !a.valid ) return finish( a );
		const EvaluationResult b = EvaluateInput( node->inputs[1], stack );
		if( !b.valid ) return finish( b );
		if( !std::holds_alternative<float>( a.value ) || !std::holds_alternative<float>( b.value ) ) return finish( Error( "La operacion necesita valores float." ) );
		const float left = std::get<float>( a.value );
		const float right = std::get<float>( b.value );
		switch( node->kind )
		{
			case NodeKind::Add: return finish( FloatResult( left + right ) );
			case NodeKind::Subtract: return finish( FloatResult( left - right ) );
			case NodeKind::Multiply: return finish( FloatResult( left * right ) );
			default: return finish( Error( "Tipo de nodo no evaluable." ) );
		}
	}
	...
	EvaluationResult NodeGraph::EvaluateInput( const NodePin& pin, std::vector<NodeId>& stack ) const
	{
		...
		if( !link )
		{
			if( pin.type == NodeValueType::Float ) return FloatResult( 0.0f );
			if( pin.type == NodeValueType::Vector3 ) return VectorResult( {} );
			return TextureResult( {} );
		}
		const NodePin* output = FindPin( link->outputPin );
		if( !output ) return Error( "La conexion apunta a un pin inexistente." );
	...
	bool NodeGraph::WouldCreateCycle( NodeId sourceNode, NodeId destinationNode ) const
	{
		...
		while( !pending.empty() )
		{
			...
			for( const GraphLink& link : links_ )
			{
				...
				if( output && input && output->nodeId == current ) pending.push_back( input->nodeId );
			}
		}
		return false;
	}
	...
	const char* NodeKindName( NodeKind kind ) noexcept
	{
		switch( kind )
		{
			case NodeKind::FloatConstant: return "Constante float";
			case NodeKind::Add: return "Suma";
			case NodeKind::Subtract: return "Resta";
			case NodeKind::Multiply: return "Multiplicacion";
			case NodeKind::ComposeVector3: return "Montar Vector3";
			case NodeKind::VectorLength: return "Longitud Vector3";
			case NodeKind::TextureSource: return "Textura de ejemplo";
			case NodeKind::TextureCpuModifier: return "Modificar textura CPU";
			case NodeKind::TexturePreview: return "Visualizar textura";
		}
		return "Nodo";
	}
	...
	const char* NodeValueTypeName( NodeValueType type ) noexcept
	{
		switch( type )
		{
			...
			case NodeValueType::Vector3: return "Vector3";
			case NodeValueType::Texture2D: return "Texture2D";
		}
		return "?";
	}
	...
	std::string NodeValueText( const NodeValue& value )
	{
		...
		if( const auto* scalar = std::get_if<float>( &value ) ) stream << *scalar;
