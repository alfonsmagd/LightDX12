...
namespace ldx12
{
	...
	void StagingDevice::BufferSubData( BufferResource& buffer, size_t dstOffset, size_t size, const void* data )
	{
		...
		if( previousState != D3D12_RESOURCE_STATE_COPY_DEST )
		{
			const CD3DX12_RESOURCE_BARRIER barrier =
				CD3DX12_RESOURCE_BARRIER::Transition( buffer.resource_.Get(), previousState, D3D12_RESOURCE_STATE_COPY_DEST );
		...
		{
			const CD3DX12_RESOURCE_BARRIER barrier =
				CD3DX12_RESOURCE_BARRIER::Transition( buffer.resource_.Get(), D3D12_RESOURCE_STATE_COPY_DEST, previousState );
		...
		manager_.AddDeferredRelease( handle, [ staging = std::move( stagingBuffer ) ]() mutable { staging.Reset(); } );
	}
	...
	void StagingDevice::TextureSubData( TextureResource& texture, const void* data, uint32_t rowPitch, uint32_t slicePitch )
	{
		...
		{
			throw std::runtime_error( "TextureSubData supports only Texture2D, Texture2DArray and TextureCube uploads." );
		}

		UINT64 uploadBufferSize = 0;
		std::unique_ptr<D3D12_PLACED_SUBRESOURCE_FOOTPRINT[]> layouts = std::make_unique<D3D12_PLACED_SUBRESOURCE_FOOTPRINT[]>( subresourceCount );
		...
		manager_.device_->GetCopyableFootprints( &texture.desc_, 0, subresourceCount, 0, layouts.get(), rowCounts.get(), rowSizes.get(), &uploadBufferSize );

		for( uint32_t subresource = 0; subresource < subresourceCount; ++subresource )
		{
			if( rowPitch < rowSizes[ subresource ] )
			{
				throw std::runtime_error( "TextureSubData rowPitch is smaller than the required source row size." );
			}

			UINT64 requiredSlicePitch = rowSizes[ subresource ];
			if( rowCounts[ subresource ] > 1u )
			{
				requiredSlicePitch += static_cast<UINT64>( rowCounts[ subresource ] - 1u ) * static_cast<UINT64>( rowPitch );
			}

			if( slicePitch < requiredSlicePitch )
			{
				throw std::runtime_error( "TextureSubData slicePitch is smaller than the required source data size." );
			}
		}
		...
		for( uint32_t subresource = 0; subresource < subresourceCount; ++subresource )
		{
			const uint8_t* sourceSlice = srcBytes + static_cast<size_t>( subresource ) * slicePitch;
			const D3D12_PLACED_SUBRESOURCE_FOOTPRINT& layout = layouts[ subresource ];
			for( UINT row = 0; row < rowCounts[ subresource ]; ++row )
			{
				std::memcpy( dstBytes + layout.Offset + static_cast<size_t>( row ) * layout.Footprint.RowPitch,
		...
		{
			const CD3DX12_RESOURCE_BARRIER barrier =
				CD3DX12_RESOURCE_BARRIER::Transition( texture.resource_.Get(), previousState, D3D12_RESOURCE_STATE_COPY_DEST );
			cmd.commandList_->ResourceBarrier( 1, &barrier );
		...
		{
			const CD3DX12_RESOURCE_BARRIER barrier =
				CD3DX12_RESOURCE_BARRIER::Transition( texture.resource_.Get(), D3D12_RESOURCE_STATE_COPY_DEST, previousState );
			cmd.commandList_->ResourceBarrier( 1, &barrier );
	...
	void StagingDevice::TextureData2D( TextureResource& texture, void* outData, uint32_t rowPitch, uint32_t slicePitch )
	{
		...
		{
			const CD3DX12_RESOURCE_BARRIER barrier =
				CD3DX12_RESOURCE_BARRIER::Transition( texture.resource_.Get(), previousState, D3D12_RESOURCE_STATE_COPY_SOURCE );
			cmd.commandList_->ResourceBarrier( 1, &barrier );
		...
		{
			const CD3DX12_RESOURCE_BARRIER barrier =
				CD3DX12_RESOURCE_BARRIER::Transition( texture.resource_.Get(), D3D12_RESOURCE_STATE_COPY_SOURCE, previousState );
			cmd.commandList_->ResourceBarrier( 1, &barrier );
			texture.currentState_ = previousState;
		}
		...
		{
			std::memcpy( dstBytes + static_cast<size_t>( row ) * rowPitch,
