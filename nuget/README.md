...
The package is intended for native x64 Visual Studio 2022 projects. It contains the public Ldx12 and Ldx12Utils headers, both static libraries for Debug and Release, and the MSBuild integration files. It does not contain implementation `.cpp` files.
...
```powershell
./nuget/Pack.ps1 -Version 0.2.0-local -NuGetExe C:/Tools/NuGet/nuget.exe
./nuget/TestPackage.ps1 -Version 0.2.0-local -NuGetExe C:/Tools/NuGet/nuget.exe
```

The package is written to `packages/Ldx12.0.2.0-local.nupkg`. The test extracts it through NuGet and compiles a native VS2022 consumer of Ldx12 and Ldx12Utils in both Debug and Release.
...
The existing workflow builds the package, verifies the VS2022 Debug and Release consumers, obtains a short-lived NuGet.org credential through OIDC and publishes `Ldx12.0.2.0.nupkg`. Branches and pull requests only build and test a prerelease package; they never publish it.
...
- the public `.hpp` files in the Visual Studio project model under `Ldx12 Headers` and `Ldx12 Utils Headers`;
- C++20 and Ldx12 public preprocessor definitions;
- `Ldx12d.lib` and `Ldx12Utilsd.lib` for Debug, or `Ldx12.lib` and `Ldx12Utils.lib` for Release;
