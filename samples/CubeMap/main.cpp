...
namespace
{
	...
	CubeMapPixels LoadCubeMapPixels( const std::filesystem::path& directory )
	{
		...
		for( uint32_t face = 0; face < ourCubeMapFaceCount; ++face )
		{
			...
			std::memcpy(
				cubeMap.pixels.data() + static_cast<size_t>( face ) * cubeMap.slicePitch,
				image.pixels.data(), cubeMap.slicePitch );
		}
		return cubeMap;
	}
	...
	RenderPipelineState CreateObjectPipeline( RenderDevice& device, DXGI_FORMAT colorFormat )
	{
		...
		desc.vertexShader = HLSLLoader::LoadStage(
			"shaders/CubeMapObject.hlsl", "vs_6_6", "VSMain" );
		desc.fragmentShader = HLSLLoader::LoadStage(
			"shaders/CubeMapObject.hlsl", "ps_6_6", "PSMain" );
	...
	RenderPipelineState CreateSkyboxPipeline( RenderDevice& device, DXGI_FORMAT colorFormat )
	{
		RenderPipelineDesc desc{};
		desc.vertexShader = HLSLLoader::LoadStage(
			"shaders/CubeMapSkybox.hlsl", "vs_6_6", "VSMain" );
		desc.fragmentShader = HLSLLoader::LoadStage(
			"shaders/CubeMapSkybox.hlsl", "ps_6_6", "PSMain" );
		desc.color[ 0 ].format = colorFormat;
		desc.depthFormat = DXGI_FORMAT_D32_FLOAT;
		desc.rasterizerState.CullMode = D3D12_CULL_MODE_NONE;
		desc.depthStencilState.DepthEnable = TRUE;
	...
	PushConstants BuildPushConstants( AppState& app, float time )
	{
		...
		XMStoreFloat4x4(
			&constants.skyViewProjection, XMMatrixTranspose( skyView * projection ) );
...
int WINAPI wWinMain( HINSTANCE instance, HINSTANCE, PWSTR, int showCommand )
{
	try
	{
		...
		HWND window = CreateWindowExW(
			0, windowClass.lpszClassName, L"Ldx12 - Cube Map",
			WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT,
			static_cast<int>( initialWidth ), static_cast<int>( initialHeight ),
		...
		const std::chrono::steady_clock::time_point animationStart =
